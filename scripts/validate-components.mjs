...
assert(updateRules?.content.includes('王权篇变量更新规则 v1.7.0'), '变量更新规则缺少 v1.7.0 版本头');
...
assert(stageOutline?.content.includes('王权篇阶段剧情 v2.4.0'), '阶段剧情缺少 v2.4.0 版本头');
...
for (const latePlotPoint of ['开始怀疑王权富贵数次留妖并非单纯懈怠', '着手探查他变化的原因', '很快循着秘密来往与蛛丝画等痕迹发现清瞳这只小妖', '两条救人路线共同硬约束', '父亲不阻拦与战斗只发生在大殿外']) {
...
assert(wangquanBayePersona?.content.includes('王权霸业配角人设 v1.2.0'), '王权霸业人设缺少 v1.2.0 版本头');
assert(wangquanBayePersona.constant === false && wangquanBayePersona.enabled === true, '王权霸业人设应为启用的关键词触发条目');
for (const fatherBoundary of ['不会亲自阻拦，也不会让战斗在大殿内爆发', '留在原处看着两人跨出殿门', '山庄其他力量造成的阻拦与战斗只能发生在大殿外']) {
